...
# delete the .agave.archive cache so we stage all these file back upon archiving.
# this gives us consistent data to access and validate regardless
# of the state of the job.
sed -i 's/.agave.archive//g' .agave.archive
...
echo 'y' | ./random_tree_gen.sh deep_tree 

find ./deep_tree > deep_tree_manifest.txt
