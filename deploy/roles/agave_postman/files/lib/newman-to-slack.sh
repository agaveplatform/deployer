...
main () {
    ...
    # call newman
    local output=$(docker run --rm=true -v $(pwd):/etc/newman -t $NEWMAN_IMAGE run $collection -e $environment $url $global $additional $newman_args)
