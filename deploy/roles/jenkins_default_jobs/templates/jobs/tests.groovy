...
job("tests/auth/admin-api-tests") {
	...
	steps {
		shell '''
		  ansible-playbook -i $JENKINS_HOME/agave-deployer/inventory/sandbox.yml $JENKINS_HOME/agave-deployer/deploy/run_admin_tests.plbk
      '''.stripIndent().trim()
	}
	configure {
...
job("tests/core/core-postman-tests") {
	...
	steps {
		shell '''
		      ansible-playbook -i $JENKINS_HOME/agave-deployer/inventory/sandbox.yml $JENKINS_HOME/agave-deployer/deploy/run_postman_tests.plbk
      '''.stripIndent().trim()
	}
	publishers {
		archiveJunit("$JENKINS_HOME/agave-deployer/deploy/tmp/agave_postman_tester*/reports/*.xml") {
			healthScaleFactor(1.0)
			allowEmptyResults(false)
		}
	}
